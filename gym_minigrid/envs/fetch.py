...
class FetchEnv(MiniGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        randintt = randint(0, len(objs)-1)
        print(randintt)
        target = objs[randintt]
        print("len(objs), target", len(objs), target.type, target.color)
    ...
    def step(self, action):
        ...
        print("reward and done, self.carrying:", reward, done, self.carrying)
        if self.carrying:
