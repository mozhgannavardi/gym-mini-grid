...
class GoToDoorEnv(MiniGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        '''
        ######################################################################
        # make sure we have a blue door in the env
        # because we want to give a constant mission to agent: go to blue door
        if 'blue' not in doorColors:

            doorIdx_blue = self._rand_int(0,len(doorColors))
            doorColors[doorIdx_blue] = 'blue'
        # if there is a blue door in doors just return its index
        else:
            doorIdx_blue = doorColors.index('blue')
        #print("door colors", doorColors)


        # give a constant mission to the agent: go to the blue door
        self.target_pos = doorPos[doorIdx_blue]
        self.target_color = doorColors[doorIdx_blue]
        ######################################################################
        '''
